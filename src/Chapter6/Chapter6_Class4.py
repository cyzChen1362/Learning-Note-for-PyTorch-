# ********************************************************************************
# 为什么循环神经网络中常常使用sigmoid和tanh作为激活函数，而不是ReLu？
# https://chatgpt.com/s/t_68ca1773312881919b133f71a59b270e
# ********************************************************************************

# ********************************************************************************
# RNN反向传播梯度计算通式展开
# https://chatgpt.com/s/t_68ca21ca4d10819191dabd988ee1aa52
# 从这里就可以发现RNN的梯度消失问题了：
# 当然，这个梯度消失并不是多层感知机里面的那个算梯度算出来极小值的梯度消失
# 而是这个参数仍然有梯度，但是远方的信号对这个参数的梯度消失了
# 毕竟某项高次方，如果W小于1的话，W^n就趋近于0了
# 也就是ht-n对ht几乎没影响，被忘掉了
# 这也就是为什么下面乱七八糟歌词生成器，两个词如果相隔距离较远，则几乎没有联系的根本原因
# 梯度爆炸可以用裁剪梯度的方法解决，但梯度消失没法解决
# 这就是GRU和LSTM出来的原因了
# ********************************************************************************
